...
const Movies = () => {
  ...
  const fetchMovies = async (category, page = 1) => {
    ...
    const response = await fetch(
      `https://api.themoviedb.org/3/tv/${updatedCategory.toLowerCase()}?api_key=${apiKey}&language=en-US&page=${page}&per_page=${moviesPerPage}`,
    )
    if (response.ok) {
  ...
  const renderMovieItem = ({ item }) => {
    const handleMoreDetails = () => {
      navigation.navigate('TvDetails', { movieId: item.id })
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        <TouchableOpacity style={styles.dropdownModal} onPress={toggleDropdown}>
